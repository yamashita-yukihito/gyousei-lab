...
2026年7月27日に、release manifestの厳密件数検証を加え、直近10年の全6科目569問を本番bundleへ掲載した。行政法220問は旧bundleと完全一致することを確認済み。同日、カード×原問監査schemaを`card-frequency-audit@3`（科目別scope）へ一般化したうえで、民法25論点の○×カードを追加した（起草と独立再監査は別AIで実施、pass 14・修正11・fail 0）。⑤に通常択一の肢を1本も取れないカード4件は、見出しごと非表示にする。UI初回はoverview、学習カード、弱点分析projectionを読み、APIは各ページを最後まで取得する。詳細は `ARCHITECTURE.md` の「多科目化のスケーラビリティ基盤」を参照する。
...
- ⑤に使う関連過去問肢: 重複を除いて276件、各カードへの表示は延べ332件
- ⑥「似た制度・他分野との違い」: 37カード
- Claude監査候補: 20件
- 類似候補: 588組
...
- 組合せ・個数・記述式の元問題は⑤の表示用の肢にせず、頻度だけへ反映する。表示できる通常択一の肢が1本もないカードは、⑤を見出しごと非表示にする（民法4件が該当）。
