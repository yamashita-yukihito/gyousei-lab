...
> 直近10年の全6科目569問は「過去問原文」と「記述式」へ掲載済みです。○×学習カードは167件で、**全6科目がそろっています**（行政法58・基礎知識32・民法25・憲法21・商法21・基礎法学10）。まず `AGENTS.md`、`HANDOFF.md`、`ARCHITECTURE.md`、`authoring/README.md`、`docs/SESSION_HANDOFF_20260729.md` を読んでください。
>
...
**当時見つかった穴**: 商法の機関・株式・計算がカード0枚だった。この穴は同日の全体精査で、取締役会の特別利害関係人、監査役会、監査役の選解任、議決権制限種類株式、分配可能額、取締役会議事録の6枚を追加して一部解消した。得点計画上、商法は広げすぎず、残りは回答履歴か明確な頻出漏れに応じて判断する。憲法総論・改正と基礎法学の法令の効力・形式は、引き続き実際の誤答を見て補う。
...
**未着手**: 行政法・民法以外の科目全体を覆う頻出度の独立再監査、⑤の`currentLawAsOf`が17肢だけ2026-07-17である点、release guardの科目別比較。Bの定型句は全体精査で除去し、件数ドキュメントも更新した。Mac外バックアップは2026-07-29にWindows共有へ作成済みで、今後は定期バックアップの成否を確認する。
...
- 過去問: 569問
  ...
  - 多肢選択式: 30問
  - 記述式: 30問
- ○×学習カード: 167件（行政法58・基礎知識32・民法25・憲法21・商法・会社法21・基礎法学10）
  - ○×の内訳: ○73件・×94件（民法52%・憲法52%・基礎法学50%・基礎知識44%・商法43%・行政法36%）
- ⑤に使う関連過去問肢: 重複を除いて628件、各カードへの表示は延べ701件
