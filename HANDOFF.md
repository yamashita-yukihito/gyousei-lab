...
> 直近10年の全6科目569問は「過去問原文」と「記述式」へ掲載済みです。○×学習カードは98件（行政法55・民法25・基礎知識18）です。まず `AGENTS.md`、`HANDOFF.md`、`ARCHITECTURE.md`、`authoring/README.md`、`docs/SESSION_HANDOFF_20260728.md` を読んでください。
>
> 次は回答時の自信度の記録、基礎知識の残り（戸籍法・住民基本台帳法・情報通信）、科目横断⑥の整備、商法・会社法の頻出15枚へ進んでください。優先順位の根拠は `ARCHITECTURE.md` の「1.5 得点計画」と「12. 次に行う作業」です。
>
...
- 過去問: 569問
  ...
  - 多肢選択式: 30問
  - 記述式: 30問
- ○×学習カード: 98件（行政法55・民法25・基礎知識18）
- ⑤に使う関連過去問肢: 重複を除いて334件、各カードへの表示は延べ391件
- ⑥「似た制度・他分野との違い」: 38カード
- Claude監査候補: 20件
- 類似候補: 588組
