...
> 直近10年の全6科目569問は「過去問原文」と「記述式」へ掲載済みです。○×学習カード55件は行政法だけです。まず `AGENTS.md`、`HANDOFF.md`、`ARCHITECTURE.md`、`authoring/README.md` を読んでください。
>
> 次はカード×原問監査schemaを全科目向けに一般化し、その後に民法20〜30論点の○×カードへ進んでください。直前作業でUIを変更しているため、ブラウザを利用できる環境ではPC幅とスマホ幅を再確認してください。
>
...
2026年7月27日に、release manifestの厳密件数検証を加え、直近10年の全6科目569問を本番bundleへ掲載した。行政法220問は旧bundleと完全一致することを確認済み。UI初回はoverview、学習カード、弱点分析projectionを読み、APIは各ページを最後まで取得する。カード内の回答回数表示と苦手`studyView`変更後の実ブラウザ確認は未実施。詳細は `ARCHITECTURE.md` の「多科目化のスケーラビリティ基盤」を参照する。
...
- 法令基準日: 2026-04-01
- bundle revision:
  `a04182c39d6dbe14fa4183bcf7a2ae61e13a4b31e4def1ae859bcae3f67fca32`
...
- `static/app.js` の版は `20260727-2`。UI資産を変更する時はHTMLのクエリ版と一緒に更新する。
