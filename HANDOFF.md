...
> 直近10年の全6科目569問は「過去問原文」と「記述式」へ掲載済みです。○×学習カードは158件で、**全6科目がそろっています**（行政法55・基礎知識33・民法25・憲法20・商法15・基礎法学10）。まず `AGENTS.md`、`HANDOFF.md`、`ARCHITECTURE.md`、`authoring/README.md`、`docs/SESSION_HANDOFF_20260728.md` を読んでください。
>
...
**残っている穴**（次に埋めるべき）: 商法の**機関47問/20年度・株式46問/20年度・計算11問/9年度がカード0枚**。「問36と問37だけが枠として固定されている」という2026-07-29の判断は不正確で、実際は問38が株式、問39が機関、問40が計算・組織再編で、直近10年はいずれも枠が安定している。株主総会の決議要件、取締役会の招集・決議、監査役の権限、剰余金配当の財源規制は20年で4〜5回ずつ出ており、5〜6枚足す価値がある。憲法の憲法総論・改正（7問/5年度）と、基礎法学の法令の効力・形式（14問/12年度がカード1枚）も薄い。
...
- 過去問: 569問
  ...
  - 多肢選択式: 30問
  - 記述式: 30問
- ○×学習カード: 158件（行政法55・基礎知識33・民法25・憲法20・商法・会社法15・基礎法学10）
  - ○×の内訳: ○68件・×90件（民法52%・憲法50%・基礎法学50%・基礎知識42%・商法40%・行政法36%）
- ⑤に使う関連過去問肢: 重複を除いて599件、各カードへの表示は延べ670件
- ⑥「似た制度・他分野との違い」: 64カード（うち科目をまたぐ関連付け22件）
