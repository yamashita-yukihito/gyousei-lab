...
2026年8月3日に、**旧10年の肢を⑤へ出せるようにし、カードを4枚足して214枚にした**
（bundle revision `c5f17ceb…`）。「平成18年度以降に2回以上出ている論点はカード化する」という
線引き（`AGENTS.md` 5.3）に合わせるための土台変更である。

平成18〜27年度は取得元がURLも解説も出していないので、次の形に限って⑤へ出す。

- ⑤の出典側の記録へ `sourceEra: "archive"` を立て、`providerUrl` と `officialQuestionUrl` を持たせない。
  **URLを省けるのは `sourceEra` が `archive` のときだけ**で、それ以外の年度は今までどおりURL必須である
  （`production_bundle.py` とテストの両方で担保）。
- `examEvaluation` は当時の正解番号から、`currentEvaluation` は**e-Govの条文（2026-04-01）で確かめた**。
  `verification` は `egov_recheck_2026-04-01`。
- 画面は出典リンクを出さず、「平成◯年・問◯・肢◯」と**「現行法で再確認済み」**の印を出す。
  元問題欄も、リンクの代わりに「元問題 平成◯年 問◯ 肢◯（現行法で再確認済み）」と文字で出す。

作ったカードは次の4枚。⑤の肢8本はすべて旧10年で、e-Govで現行法の正誤を確かめた。

| カード | 論点 | 出題 |
|---|---|---|
| `min-inheritance-statutory-share-001` | 法定相続分と養子の相続分（900条・809条） | 平成19年問35・平成24年問35 |
| `min-inheritance-substitution-cause-001` | 代襲相続の原因（887条2項・891条・939条） | 平成19年・21年・24年問35 |
| `min-inheritance-renunciation-effect-001` | 相続放棄の効果と詐害行為取消権（939条） | 平成19年問35・平成25年問30 |
| `min-contract-leased-land-act-scope-001` | 借地借家法が適用される範囲（地上権・公営住宅） | 平成18年問30・平成20年問10 |

**遺留分（1042条・1046条）だけは作れていない。** 2回のうち1回（平成24年問46）が記述式で、
記述式は⑤へ出さない決まりのためである。

判例URLは、最判昭和49年9月20日（相続放棄と詐害行為取消権）と最判昭和59年12月13日
（公営住宅と信頼関係の法理）を、実際に開いて事件名と裁判年月日が一致することを確かめてから入れた。
最初に書いたURLは2つとも別の判例を指していた。

2026年8月3日に、**FP2級とかぶる論点を6枚足して210枚にした**（bundle revision `a2df31bb…`）。
利用者は8月末にFP2級実技、9月に学科を受ける。かぶり分野は
`~/dev/yuki-services/apps/sakura-tools/authoring/fp2/fp2_gyosei_overlap_src/` に47論点＋追加7論点
としてまとまっており、そこから**行政書士の直近10年に肢があるものだけ**を選んだ。

| カード | 論点 | 出典 |
|---|---|---|
| `min-inheritance-joint-will-001` | 共同遺言の禁止（975条） | 令和5年問35肢ウ |
| `min-inheritance-will-revocation-001` | 遺言の撤回の方式（1022条） | 令和5年問35肢オ |
| `min-inheritance-ward-will-001` | 成年被後見人の遺言（973条） | 令和5年問35肢ア |
| `min-inheritance-legatee-predecease-001` | 受遺者の先死亡と遺贈の失効（994条1項） | 令和5年問35肢エ |
| `min-contract-loan-for-use-rescission-001` | 使用貸借の借主による解除（598条3項） | 令和5年問33肢ア |
| `min-general-voluntary-guardianship-001` | 任意後見と法定後見の代理権（859条・876条の9） | 令和7年問28肢ア |

**6枚ともAが過去問の肢の原文と一致している。**

かぶり分野47論点のうちカードにできたのは、既存分と合わせて**16枚ぶん**である。
できなかったものと理由は `docs/FP2_OVERLAP_CARDS.md` にまとめた。要点は2つ。

- **相続分・代襲相続・借地借家法は、FPでは主力なのに行政書士の直近10年に肢が無い。**
  相続枠（問35）は遺言の方式・遺産分割・配偶者居住権に寄っている。
  FPでやり込んでも行政書士の得点には直結しないので、FP側のページで割り切って回す。
- **区分所有法・都市計画法・不動産登記法の手続は行政書士試験の範囲外。**
  かぶり分野の資料自身がそう書いている。

かぶり分野は民法と基礎知識に集中していて、**行政法（112点・最大の得点源）とは重ならない**。
