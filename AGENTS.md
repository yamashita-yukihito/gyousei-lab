...
- **Aは過去問の肢の原文をそのまま使う。** 装飾記法を足すのはよいが、**記法を外した文字列が⑤の肢と1文字も違わない**こと。条文や判例を自分の言葉で一般命題へ言い換えたAを新しく作らない。本番で問われるのは肢の書きぶりそのものなので、そこごと覚える。
  ...
  - 2026-08-03時点で、219枚中**156枚（71%）**が一致している。作業前は81枚だった。残り63枚は上の(1)〜(4)に当たるか、⑤にその論点を正面から問う肢が無いものである。
...
- **スクリプトでカードを足したり直したりしたら、bundleを作る前に `card_edit.validate_card` を全カードへ通す。** `gyousei-production-bundle` はこの検証を呼ばないので、装飾の入れ子や閉じ忘れがあってもbundleは作れてしまう。テストも `tests/` を走らせてから差し替える。
