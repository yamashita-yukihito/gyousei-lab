...
- **出す位置は「正しい形で覚えると」（`correction`）の直後で固定**。カード側からは選べない。2026-07-31に一度は選べるようにしたが、深掘りは3列の格子なので、そこへ図を入れると1枠だけ横に伸びて右側が空き、レイアウトが崩れた。位置を増やさない。
...
- **Aは過去問の肢の原文をそのまま使う。** 装飾記法を足すのはよいが、**記法を外した文字列が⑤の肢と1文字も違わない**こと。条文や判例を自分の言葉で一般命題へ言い換えたAを新しく作らない。本番で問われるのは肢の書きぶりそのものなので、そこごと覚える。
  - 例外は、**⑤にその論点を正面から問う肢が無いとき**（条文の要件を一つずつ確かめる型の論点、記述式から作った論点など）。そのときだけ自作の命題でよい。無理に別の肢へ寄せて、カードの論点そのものを変えない。
  - **そのまま使えない場合**は、次のどれかである。(1) 肢が `A・B・C` や `甲・乙・丙` で当事者を指しており、設問側の事案設定が無いと読めない。(2) 肢が列挙の断片で、文になっていない。(3) 原文に注番号（`*4` など）が入っている。(4) カードが複数の肢や条文をまとめており、どの肢とも1対1にならない。この4つに当たるときだけ自作の命題でよい。
  - **候補を選ぶときは、旧Aといちばん重なる肢を選ぶ。** 短い肢を選ぶと、旧Aが持っていた限定（「その根拠となる規定が法律に置かれているものに限る」など）や、subtopic が指す論点の一部が落ちる。
  - 2026-07-31時点で、187枚中**125枚（66%）**が一致している。作業前は81枚だった。残り62枚は上の(1)〜(4)に当たるか、⑤にその論点を正面から問う肢が無いものである。
...
- 深掘り解説は開閉式に戻さず、回答後に普通に表示する。**「制度の背景・理由」「試験のひっかけ」「具体的な場面」の3つは縦に積む。** 2026-07-31に3列をやめた。分量が大きく違ううえ（背景は63枚が300字超・最大891字）、3列だと幅330pxへ13pxの本文を詰め込む形になり、いちばん読ませたい部分がいちばん読みにくかった。本文の文字サイズは①普通の解説・④常識力と同じにそろえる。3列へ戻さない。

- 各カードには「編集」「書き出し」「取り込み」を出す（2026-07-31に追加）。**解く前から押せるよう、両方の画面でカードIDのすぐ下に置く。** 回答後の欄へ戻さない。開く前の確認は入れない（編集画面に答えが出るのは押す前から分かることなので、毎回聞くのは邪魔になる）。
- 画面からの保存は `POST /api/card-source` が正本 `content/explanation_cards.json` を書き換え、bundleを作り直して差し替える。**検証は `card_edit.py` が正本**で、`authoring/tools/card_exchange.py` と同じものを通す。片方だけ緩めない。
- 保存に使う口はこの1つだけにする。`editable` に無い項目（`frequency`、`sourceRefs`、`relatedPastQuestions`、`review`、ID類）は画面から書き換えさせない。
- 作り直しに失敗したら正本を書き戻す。**画面に出ない内容を正本へ残さない。** 件数のfail closed検証は稼働中bundleと同じ数を要求する（この口はカードを増減しない）。
- 取り込みは必ず確認画面をはさみ、そのあと編集画面へ値を入れて開く。**取り込んだだけでは保存しない。**

### 5.1 判例の結論だけを覚える論点（深掘りを増やさない）

行政事件訴訟法の**処分性・原告適格・狭義の訴えの利益**は、制度趣旨を積み上げても得点にならず、
**どの事案が○でどの事案が×か**を判例ごとに覚えるほうが速い。利用者の方針として、
...
- **深掘りを長くしない。** 「制度の背景・理由」は、○×を分ける一言で足りる。判例の事実関係、
  学説、他判例との横断比較を足して情報量を増やさない。
- **Aは過去問の肢の原文をそのまま使う。** 抽象化した一般命題へ言い換えない。本番で問われるのは
  事案そのものなので、事案の書きぶりごと覚える。
- `correction` は「どの語が判定を分けるか」を条文・判例番号付きで短く書く。
- ⑥は、原告適格と狭義の訴えの利益のように**取り違えやすいものの区別**だけに使う。
  区別は暗記の助けになるので消さない。理解を深めるための比較は足さない。
- **この方針をカードの本文へ書かない。** 「このカードは暗記型です」「深掘り改修不要」といった
  編集者向けの申し送りは、画面に出る欄（`explanations`・`evidenceHighlights` など）へ入れない。
  方針はここに書き、カードには学習者が読む内容だけを置く。

行政法の他の分野、行政手続法・行政不服審査法・民法などは従来どおり、理解を助ける深掘りを書く。
