...
- 民法・固定配役ドリルA案: `static/minpo-a/`（問題の正本は同ディレクトリの `problems.json`）
- API: `server.py`
- テスト: `tests/`
- 問題生成コード: `authoring/`
...
入口は `static/architecture.html` の「engineに載せない、独立した4ページ」に置いた。
利用者の指示でタブへは並べない。
...
### 5.2 民法・固定配役ドリル（`static/minpo-a/`）

民法の事例問題で人物設定を読み解く時間をなくすための独立ページ。
**A案・B案・C案を作って使い比べる試作**なので、学習エンジンへ統合しない。
LANから `http://192.168.10.102:8080/services/gyousei-lab/minpo-a/` で開ける。
...
- 問題の正本は `static/minpo-a/problems.json`。**事例文は自分で書く。**
  合格道場の問題文・解説を貼らない。だから `static/` に置いてWebから読ませてよい。
- 記録はブラウザのlocalStorage（`minpoA:v1`）だけに持つ。`production.sqlite3` へ書かない。
  学習カードの回答履歴と混ぜない。
...
- **A案・B案・C案の3つを並べて使い比べている。** どれも `static/` の下の独立ページで、
  記録はそれぞれ `minpoA:v1` / `minpoB:v1` / `minpoC:v1` のlocalStorageだけに持つ。
  ...
  - 互いのファイルを読ませない。データ形式も別のままにする。**まだ1つに統合しない。**
...
- 3案を1つに寄せる場合の設計は `docs/MINPO_ABC_REVIEW.md` の7章に書いた。
  そのときは**事案の時系列を確定する／条文はe-Govと機械照合してから書く／術語を日常語の意味で使わない／
  ○×は章ごとに5:5へ寄せる**の4つを最初から守る。3案の失敗がそのまま根拠になっている。
