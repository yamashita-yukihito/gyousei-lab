...
- 各カードには「編集」「書き出し」「取り込み」を出す（2026-07-31に追加）。**解く前から押せるよう、両方の画面でカードIDのすぐ下に置く。** 回答後の欄へ戻さない。
- ただし編集画面には正解と解説が出る。○×学習でまだ答えていないカードの「編集」「取り込み」は、開く前に一度確かめる。「書き出し」はファイルになるだけなので確かめない。
