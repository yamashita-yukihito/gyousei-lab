...
**A案とB案を作って使い比べる試作**なので、学習エンジンへ統合しない。
...
- **B案には未修正の誤りが5件ある。** レビューは `docs/MINPO_ABC_REVIEW.md`。
  `scene-001-s3` は当事者関係が成立しておらず、`scene-003-s4` は法定地上権の建築時期を確定していない。
  条文番号3件（1043条→1049条、土地の事案への395条、民事執行法93条→180条2号）も誤り。
  **直す前に、この一覧の該当箇所も更新する。**
