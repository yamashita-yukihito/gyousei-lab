...
- `architectureVersion`: `2026-07-29`
- 更新日: 2026-07-29 JST
- 人間向け構成図: `static/architecture.html`
- AI向け正本: このファイル

このラボは、2026年11月8日の行政書士試験まで約3か月かけて、本人が問題を解きながら教材と分析機能を育てる長期プロジェクトである。直近10年の過去問は全6科目を一つのアプリに掲載済み。○×学習カードは**全6科目187件**を掲載済みである（行政法65、基礎知識32、民法30、憲法24、商法・会社法24、基礎法学12）。
...
- **頻出度は全187枚が監査JSONへ統合されている**（2026-07-29）。最初の全科目再監査は行政法55・民法25の80枚と、憲法24・基礎知識32・商法21・基礎法学12の89枚、計169枚を覆っていた。その後、未収録だった行政事件訴訟3枚を全20年から再監査し、行政法7・民法5・会社法3の新規15枚とともに統合した。ラベルは問題数で機械的に決める（1〜2=重要論点、3〜5=繰り返し出題、6〜9=頻出、10以上=最頻出）。基礎知識は制度変更を踏まえて直近10年を原則とし、制度導入後しか比較できない個別カードも実際に数えた期間を`scope`へ明記する。
...
- ○×学習カード187件（行政法65、基礎知識32、民法30、憲法24、商法・会社法24、基礎法学12）
  - 民法は、動産売買先取特権、事務管理、転貸借、法定地上権、生命・身体侵害の時効を追加して30枚とした
  - 基礎知識は、一回限りで変動しやすい比較問題1枚を取り下げ、制度の骨格を優先して32枚とした
...
保存素材1,139問と、本番掲載569問・187カードを混同しない。
...
学習エンジンとは別に置いた独立ページ。**A案とB案を作って使い比べるための試作**なので、
共通renderer・共通APIには載せていない。比較して勝った側の考え方だけを、あとで学習エンジンへ取り込む。
...
- 独立ページへの入口は `static/architecture.html` の「engineに載せない、独立した4ページ」。
...
比較表とD案の設計は `docs/MINPO_ABC_REVIEW.md`。B案には未修正の誤りが5件ある。
...
3. cardとcard: ⑥「似た制度・他分野との違い」。1対1の比較。2026-07-29時点で61カード、
   うち19件が科目をまたぐ関連付け（`relatedCardId`の指す先が別科目）である。
...
2026-07-31に、カード187枚の学習時間を実測から見積もった。1枚あたり初回3.6分・2〜3周目1.2分・
高速○×0.3分で、**187枚を習得まで通しても約22時間**である。一般的な必要時間（初学者800〜1,000時間、
ラムネグさんの実績で約500時間）の**4〜5%**にすぎない。カードは「論点を知る」段階の道具として
機能しているが、これだけでは足りない。以下は、その不足を埋める順に並べている。

**優先度つきの残り作業（2026-07-31更新）**

| | 内容 | 状態 |
|---|---|---|
| ① | **確信度ダッシュボード**。「理由まで言える／なんとなく／勘」を分けて、表面得点と確信正解点を別に出す。記録側（`card_marks`の`confidence`）は実装済みで、**集計と表示だけが未実装**。ラムネグ式の中心で、費用対効果がいちばん高い | データが貯まってから着手 |
| ② | 基礎知識のカバー率 | 2026-07-31に7枚追加（32→39枚）。制度枠（問52〜57）38% |
| ③ | 記述式60点（配点の20%）の本格的な訓練。**2026年9月ごろに着手する。** 記述は自分で文章を書くので、知識がある程度たまってからでないと成立しない。それまでは③'で代替する | 2026-09に着手予定 |
| ③' | **記述の論点を択一で潰す**（③までのつなぎ）。記述式30問の論点を洗い、同じ論点を問う択一の肢があればカード化する。2026-07-31に6枚追加した | 一巡した。択一に肢が無い3論点が残 |
| ④ | 民法の演習量。過去問10年で約110問しかなく、事例推理の量が足りない。過去問以外の手当てが要る | 未着手 |
| ⑤ | **民法の固定キャラクター演習ページ**。事例問題のA・B・Cを毎回読み解く時間が無駄なので、登場人物を固定の配役にし、状況設定が一目で分かる形にする | A（16問）・B（31肢）・C（27問）の3案がそろった。レビューは `docs/MINPO_ABC_REVIEW.md`。**次は実際に解いて比べ、D案の骨格を決める**。B案の誤り5件は先に直す |
| ⑥ | 商法24枚の検証、誤答理由タグ | 未着手 |
...
4. **商法・会社法24枚の検証**。設立・株式・機関・商行為・計算に絞っているので、実際の回答履歴を見て過不足を判断する。

その後に、誤答理由タグ（条文を知らなかった／要件を取り違えた／問い方を読み落とした など）、
行政法と民法の`learningRelations`、民法の次候補（錯誤、工作物責任、日常家事代理）、
全分野頻出viewを進める。転貸借、法定地上権、生命・身体侵害の消滅時効は2026-07-29に追加済み。
