...
- 「今日の学習」の使い方ガイド（人間向け）: `static/study-guide.html`
- AI向け正本: このファイル

このラボは、2026年11月8日の行政書士試験へ向けて、本人が問題を解きながら教材と分析機能を育てる長期プロジェクトである。8月は開発中心、9月から本格学習に入る。直近10年の過去問は全6科目を一つのアプリに掲載済み。○×学習カードは**全6科目246件**を掲載済みである（行政法83、基礎知識42、民法54、憲法27、商法・会社法28、基礎法学12）。
...
- ○×学習カード246件（行政法83、基礎知識42、民法54、憲法27、商法・会社法28、基礎法学12）
...
### 12.1 20年棚卸しの残りと「今日の学習」（2026-08-04に利用者が順番を決定）

**A → B → C の順で進める。** Aから着手する。

| | 作業 | 内容 | 状態 |
|---|---|---|---|
| **A** | **旧10年の本文読み** | 平成18〜27年度の未参照484問（行政法182・基礎知識108・民法81・商法28・多肢28・記述28・基礎法学15・憲法14）を読んで論点タグを付け、20年で2回以上かつカードの無いものをカード化する。旧10年は**科目名1つしかラベルが無い**ので、本文を読まないと論点で束ねられない | 着手中 |
| **B** | **肢単位の棚卸し** | いまの棚卸しは**問題単位**で「済／未参照」を見ていた。カードが1肢でも⑤に貼っていればその問題は「済」になる。現行10年の五肢問題509問2,545肢のうち、頻出度の根拠にしている346問1,730肢を見ると、**⑤に貼っていない肢が1,132本**ある（134問は1肢も貼っていない）。これを読んで論点タグを付け、234枚と突き合わせる。⑤にそのまま使えるので旧10年より軽い | 未着手 |
| **C** | **「今日の学習」キュー** | 下記の2機能で作る | **完了（2026-08-05）** |

**Cの中身（2026-08-04に利用者が指定・2026-08-05に実装）**

- **py-fsrs を使った間隔反復**（`study_queue.py`・`GET /api/study-queue`）。
  ライブラリは `fsrs` 6.x を `pip install --user` で入れてある（Homebrew Pythonの外＝
  `~/Library/Python/3.12/lib/python/site-packages`）。launchd plistは変えていない。
  - **FSRSの状態は専用テーブルへ保存せず、`card_attempts` と `card_marks` から毎回導出する。**
    `production.sqlite3` のschemaは変更していない。理由は3つで、(1)導出値を別に持つと正本と写しが
    ずれる、(2)リセットの区切りが増えるたびに計算し直しが要る、(3)数百件の規模なので毎回replayしても
    一瞬で終わる。同じ理由で `enable_fuzzing` を切り、同じ履歴なら必ず同じ期日になるようにしている。
  - **○×を4段階へ広げるのに、回答ごとの自信度を使う。** 誤答→Again、正答＋あてずっぽう→Hard、
    正答＋たぶん→Good、正答＋確信あり→Easy、正答＋記録なし→Good。自信度は出題対象の判定には
    効かせない決まりだが、**次にいつ出すか**へ効かせるのはその決まりに反しない。
  - 「絶対覚えた」はキューから外すが履歴も期日も消さない。解除すれば同じ履歴からそのまま戻る。
  - **間隔の上限を受験日までの残り日数にしている。** 切らないと、確信ありで2回正解しただけで
    次が397日後になり、本番までに二度と出てこないカードができる（2026-08-05に確認して切った）。
  - **はじめてのカードは別枠で絞る。** 期日が来た復習は絞っても溜まるだけなので全部出す。
    画面の「1日の量」で 軽め（新規6・合計20・約15分）／ふつう（12・40・約30分）／
    しっかり（25・80・約60分）／復習だけ を選ぶ。既定は軽めで、選択は端末のlocalStorageに残す。
    枚数は実測の回答時間から決めた。利用者の勉強時間は8月が1日15分、9月から1日2時間である。
  - **カードを `learningType` で暗記もの158枚／理解もの88枚に分けた**（2026-08-05）。
    8月は1日15分しかないので、暗記ものだけを回す。別アプリへは分けず、画面の
    「カードの種類」で絞る共通エンジンのfilterとして作った。回答履歴と期日は共通のまま。
  - 画面は出題範囲の選択肢に「今日の学習（間隔反復）」を足した形。
- **Ankiへの書き出し**（`authoring/tools/anki_export.py`）。**タブ区切りのテキスト1枚**を出す。
  apkgにしないのは、apkgがSQLite＋zipの内部形式でAnkiの版が変わると壊れやすいからである。
  - 列は 表／裏／タグ／カードID／頻出度／出典 の6つ。1行目にAnki用のヘッダを付けるので、
    フィールドの割り当てを毎回やり直さなくてよい。
  - **表はA・B1・B2・C、裏は正誤と解説**。ラボと同じで、表に答えを出さない。
  - 装飾記法は `card_edit.strip_markup` で外す。Ankiに色は持ち込まない。
  - ⑥⑦⑧（比較・表・図）は出さない。テキスト取り込みでは表現できないため、図はラボの画面で見る。

Bの数字の出し方は、各カードの `relatedPastQuestions[].choiceId` と問題側の肢IDを突き合わせる。
「⑤に貼っていない」は「その論点のカードが無い」と同義ではない（⑤は表示用で1枚に3〜4肢しか貼らない）ので、
1,132本は**穴の上限**として扱う。
