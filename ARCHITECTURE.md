...
このラボは、2026年11月8日の行政書士試験へ向けて、本人が問題を解きながら教材と分析機能を育てる長期プロジェクトである。8月は開発中心、9月から本格学習に入る。直近10年の過去問は全6科目を一つのアプリに掲載済み。○×学習カードは**全6科目242件**を掲載済みである（行政法83、基礎知識41、民法51、憲法27、商法・会社法28、基礎法学12）。
...
- ○×学習カード242件（行政法83、基礎知識41、民法51、憲法27、商法・会社法28、基礎法学12）
...
| **C** | **「今日の学習」キュー** | 下記の2機能で作る | 未着手 |

**Cの中身（2026-08-04に利用者が指定）**

- **py-fsrs を使った間隔反復。** 独自のスケジューラを書かず、`py-fsrs`（FSRSアルゴリズムの公式Python実装）へ寄せる。
  カードごとの状態（stability・difficulty・次回期日）を持ち、回答のたびに更新する。既存の追記型
  `card_answers` / `card_marks` を壊さず、FSRSの状態は別テーブルに置く。「卒業」「絶対覚えた」との
  関係（絶対覚えたはキューから外すが履歴は消さない）は既存ルールに合わせる。
- **Ankiへの書き出し。** Ankiアプリで回せる形（apkg または csv）へ書き出す。A・B1・B2・Cと
  回答後の解説をどうカードの表裏へ割るかは着手時に決める。⑤の肢と条文リンクも持っていく。
