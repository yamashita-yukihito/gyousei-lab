...
- `architectureVersion`: `2026-07-28`
- 更新日: 2026-07-28 JST
- 人間向け構成図: `static/architecture.html`
- AI向け正本: このファイル

このラボは、2026年11月8日の行政書士試験まで約3か月かけて、本人が問題を解きながら教材と分析機能を育てる長期プロジェクトである。直近10年の過去問は全6科目を一つのアプリに掲載済み。○×学習カードは行政法55件・民法25件・基礎知識18件を掲載済みで、憲法・商法・基礎法学へ順次拡張する。
...
### 目標の内訳（合計196〜200点を本命にする）

2026-07-28に、合格体験談9例の調査（`docs/PASS_STRATEGY_RESEARCH_WITH_RAMUNEGU.md`）を受けて改訂した。
**形式ごとに分けて数え、記述を科目の総点へ埋め込まない。** 記述は採点基準が非公表で読みにくいため、
「記述を除いていくら取れているか」を常に見えるようにしておく。

| 区分 | 目標 | 考え方 |
| --- | --- | --- |
| 行政法・択一（19問76点） | 64〜68点 | 16〜17/19。**最大の得点源で、ここを崩すと他で取り返せない** |
| 基礎知識（14問56点） | 40〜44点 | 10〜11/14。足切りは24点だが、**利用者は過去に56点満点の実績がある。足切り対策だけに使うのはもったいない** |
| 民法・択一（9問36点） | 24点 | 6/9。**深追いしない。A論点だけ理由まで、その他は制度名・要件・効果で通す** |
| 多肢選択（3問24点） | 16〜18点 | 空欄ごとに2点。**完全正解を狙わず、確実な語から埋める** |
| 記述（3問60点） | 20〜24点 | **別枠で数える。0点前提にはしないが、40点以上にも依存しない** |
| 憲法・択一（5問20点） | 16点 | 4/5。範囲が狭く、繰り返し出る |
| 商法・会社法（5問20点） | 8点 | 2/5。**20点しかないので時間を使わない。頻出だけ確実に** |
| 基礎法学（2問8点） | 4点 | 1/2。範囲が広く費用対効果が低い。深入りしない |

下限の積み上げで192点、上限で206点。**本命は196〜200点**とする。記述を除いた素点は172〜182点で、
記述が振るわない年でも合格ラインへ手が届く設計になっている。

改訂前（2026-07-27）は基礎知識36点・総合192点で、記述を行政法88点・民法40点の中へ含めていた。
...
- **商法・会社法**は会社法の設立・株式・機関と商法総則の商行為に絞り、15枚程度で打ち止めにする。
...
説明できる170点は本番での意味が違う。回答時に自信度を残せるようにし、
表面上の得点と「理由まで言える得点」を分けて表示する（未実装。次に作る機能の第一候補）。
...
- ○×学習カード98件（行政法55、民法25、基礎知識18）
  - 民法の内訳: 総則5、物権4、担保物権3、債権総論5、債権各論4、親族2、相続2
  - 基礎知識の内訳: 個人情報保護12、諸法令（行政書士法）6
- 関連過去問肢334件（延べ表示391件）
- 記述30問（行政法10、民法20）
- 類似候補588組

保存素材1,139問と、本番掲載569問・98カードを混同しない。
...
過去問の多科目化、弱点分析snapshot MVP、苦手`studyView`、カード×原問監査schemaの全科目一般化
（`card-frequency-audit@3`）、民法25論点の○×カード追加、全カードへの文字装飾、高速○×モード、
基礎知識18枚（個人情報保護12・行政書士法6）の追加は完了した。

2026-07-27に合意した優先順位のうち①装飾と②基礎知識の第一弾が終わったため、
2026-07-28に合格者調査を踏まえて次を組み替えた。根拠は「1.5 得点計画」と
`docs/PASS_STRATEGY_RESEARCH_WITH_RAMUNEGU.md`。

1. **自信度の記録**。○×の正誤とは別に「理由まで言える／なんとなく／勘」を残し、
   表面得点と確信正解点を分けて表示する。**過去問を回すほど必要になる機能で、早いほど効く。**
   `card_attempts`へ追記列を足す形にし、既存の回答revision互換は壊さない。
2. **基礎知識の残り**。戸籍法・住民基本台帳法（諸法令枠は毎年2問）、情報通信・情報セキュリティ。
   政治・経済・社会は頻出テーマだけ。
3. **科目横断⑥の整備**。民法と商法、民法と行政法など、本番で混同しやすい比較を`crossFieldComparisons`へ。
   調査資料が挙げた比較対象は、許可／認可／特許、取消し／無効／撤回、審査請求／再調査／再審査、
   無権代理／表見代理／日常家事代理、民法の表見代理／商法の表見支配人、個人情報／個人データ／保有個人データ。
4. **商法・会社法の頻出15枚**。設立・株式・機関・商行為に絞る。

その後に、誤答理由タグ（条文を知らなかった／要件を取り違えた／問い方を読み落とした など）、
行政法と民法の`learningRelations`、民法の追加論点（転貸借、法定地上権、消滅時効期間、錯誤、
工作物責任）、全分野頻出viewを進める。
