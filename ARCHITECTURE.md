...
このラボは、2026年11月8日の行政書士試験まで約3か月かけて、本人が問題を解きながら教材と分析機能を育てる長期プロジェクトである。直近10年の過去問は全6科目を一つのアプリに掲載済み。○×学習カードは行政法から民法・憲法・商法・基礎法学・基礎知識へ順次拡張する。
...
過去問の多科目化、弱点分析snapshot MVP、苦手`studyView`は完了した。

1. 民法追加前に、カード×原問監査schemaを全科目向けに一般化する。
2. 民法の優先20〜30論点を○×学習カードへ追加する。
