...
このラボは、2026年11月8日の行政書士試験まで約3か月かけて、本人が問題を解きながら教材と分析機能を育てる長期プロジェクトである。直近10年の過去問は全6科目を一つのアプリに掲載済み。○×学習カードは**全6科目151件**を掲載済みである（行政法55、基礎知識26、民法25、憲法20、商法・会社法15、基礎法学10）。
...
- **ふつうの年で190点前後**（合格ライン180点に対して10点の余裕）
- **悪い年でも170点を割らない**
- 上振れした年は200点を超える

1回の受験で100%合格させる計画ではない。**下振れを引かなければ受かる**ところまで持ち上げ、
そのうえで下振れの幅そのものを教材で狭めていく、という設計である。

### 目標の内訳（ふつうの年190点前後／悪い年でも170点）
...
| 区分 | 満点 | 悪い年（合計170） | ふつうの年（合計192） | 考え方 |
| --- | ---: | ---: | ---: | --- |
...
記述を除いた素点は、悪い年で154点、ふつうの年で172点。**記述が0点でも法令等の足切りは通る。**

### この帯の使い方

- 悪い年の列は「ここを割ったら不合格が濃厚」という**警戒線**であって、目標ではない。
- ふつうの年の列と実際の正答率の差が、そのまま**次に作る教材の優先順位**になる。
- 行政法・択一が15/19を割る状態が続くなら、他科目のカードを増やす前に行政法へ戻る。
- 商法・会社法と基礎法学は、悪い年とふつうの年で数字を変えていない。
...
- **基礎法学**は10枚で打ち止めにした（2026-07-29）。20年間、**問1が空欄補充（法史・法思想）で対策しにくく、問2が法令用語・裁判制度・法令の効力で安定している**ため、問2枠だけへ集中させた。目標1/2点はこれで足りる。
- **商法・会社法**は15枚で打ち止めにした（2026-07-29）。20年間、**問36が商法総則・商行為、問37が株式会社の設立**でほぼ固定されているため、この2枠だけへ集中させ、問38〜40（株式・機関・計算・組織再編）はカード化しない。5問中1問を確実に取るための配分である。
...
- ○×学習カード151件（行政法55、基礎知識26、民法25、憲法20、商法・会社法15、基礎法学10）
  - 民法の内訳: 総則5、物権4、担保物権3、債権総論5、債権各論4、親族2、相続2
  - 基礎知識の内訳: 個人情報保護12、諸法令（行政書士法6、住民基本台帳法・戸籍法2）
- 関連過去問肢564件（延べ表示635件）
- 記述30問（行政法10、民法20）
- 類似候補588組

保存素材1,139問と、本番掲載569問・151カードを混同しない。
...
3. cardとcard: ⑥「似た制度・他分野との違い」。1対1の比較。
4. cardと法律群: ⑦「法律ごとの結論くらべ」（`comparisonTable`）。行政手続法・行政不服審査法・
   行政事件訴訟法のように、**同じ場面で3つ以上の法律の結論が分かれる**ものを1枚の表で並べる。
   相手のカードが存在しなくても持てる点が⑥と違う。2〜4行、`label`は重複不可、装飾記法は使わない。
   `memoryCue`には並びだけでなく、なぜそう分かれるのかの理由を書く。

⑦を最初に入れたのは `gyo-aps-formal-defect-001`（申請の形式不備）で、行手法7条は「補正を求め、
または拒否」＝選択、行審法23条と行訴法7条→民訴法137条は「補正を命じなければならない」＝義務、
という対比を持つ。行政法はこの型のひっかけが繰り返し出るため、今後の行政法カードでは⑦の候補を
必ず検討する。
