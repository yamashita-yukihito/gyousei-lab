...
「180点までの距離をいちばん縮めるのはどれか」で判断する。2026-07-27に利用者と合意した。
...
### 目標の内訳（合計192点。180点に12点の余裕を持たせる）

| 科目 | 目標 | 考え方 |
| --- | --- | --- |
| 行政法 | 88点 | 択一15/19＋多肢＋記述の部分点。**最大の得点源で、ここを崩すと他で取り返せない** |
| 民法 | 40点 | 択一5/9＋記述20点。**深追いしない。記述40点があるので択一で捨てた論点が記述に出る事故を避ける** |
| 基礎知識 | 36点 | 9/14。足切りは24点だが、**利用者は過去に56点満点の実績があり、上振れを狙える科目** |
| 憲法 | 16点 | 択一3/5＋多肢。範囲が狭く、繰り返し出る |
| 商法・会社法 | 8点 | 2/5。**20点しかないので時間を使わない。頻出だけ確実に** |
| 基礎法学 | 4点 | 1/2。範囲が広く費用対効果が低い。深入りしない |
...
- **基礎知識**は範囲が閉じている分野から作る。**個人情報保護法、情報通信、行政書士法等の業務関連法令**は
  費用対効果が高い。政治・経済・社会は範囲が広いので頻出テーマに絞る。
...
2026-07-27に利用者と合意した優先順位は次のとおり。根拠は「1.5 得点計画」。

1. **全80カードへの文字装飾**（役割固定の色付け）を完了する。
2. **基礎知識のカード化**。個人情報保護法と行政書士法等の業務関連法令から始める。範囲が閉じており、
   56点の科目で上振れを狙えるため費用対効果が高い。
3. **科目横断⑥の整備**。民法と商法、民法と行政法など、本番で混同しやすい比較を`crossFieldComparisons`へ。
4. **商法・会社法の頻出15枚**。設立・株式・機関・商行為に絞る。

その後に、行政法と民法の`learningRelations`、民法の追加論点（転貸借、法定地上権、消滅時効期間、錯誤、
工作物責任）、全分野頻出viewを進める。
